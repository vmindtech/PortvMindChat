...
import { PermissionTypes, Permissions, Constants } from 'librechat-data-provider';
...
const AgentMarketplace: React.FC<AgentMarketplaceProps> = ({ className = '' }) => {
  ...
  // Set page title
  useDocumentTitle(
    `${localize('com_agents_marketplace')} | ${startupConfig?.appTitle ?? Constants.DEFAULT_APP_TITLE}`,
  );
